...
## 判官交叉校验 — 由 Jev（TypeSafe）驱动

我们用 **Jev**（TypeSafe 的 [System One 决策模型](https://typesafe.ai)，`jev-latest`）对我们的确定性判分做交叉校验。
Jev 不生成文本，而是返回带校准概率的类型化判断，因此可当独立第三方判官。
我们对一批已知金标签的构造样本，用「确定性 grader / LLM 判官 / Jev 判官」三方独立判分，报告准确率、延迟与成本，并实时展示在主页。
方法、结果与复现步骤见 [`docs/jev.md`](docs/jev.md)。
