...
const ACTOR_CLS: Record<PgActor, string> = {
  system: 'border-hairline text-dim',
  agent: 'border-ledger text-ledger',
  counterpart: 'border-amber text-amber',
};
...
function EventRow({ e }: { e: PgEvent }) {
  ...
  return (
    ...
    >
      ...
      <span
        className={`mt-0.5 shrink-0 border px-1.5 py-0.5 font-mono text-[10px] ${ACTOR_CLS[e.actor]}`}
      >
        {actorLabel}
      </span>
      ...
      {running && (
        <div className="mb-4 flex items-center gap-2.5 border border-ledger/40 bg-panel px-4 py-2.5">
          ...
          <span className="font-mono text-xs tracking-wide text-ledger">
            {t.playground.stream.negotiating}
          </span>
        </div>
      )}
      ...
      {session === null ? (
        <div className="border border-dashed border-hairline px-6 py-14 text-center">
          <p className="font-display text-base font-bold text-ink">
            {t.playground.stream.emptyTitle}
          </p>
          <p className="mt-2 text-sm text-dim">{t.playground.stream.emptyDesc}</p>
        </div>
      ) : events.length === 0 ? (
        <div className="border border-dashed border-hairline px-6 py-10 text-center font-mono text-xs text-dim">
          {t.playground.stream.waiting}
        </div>
      ) : (
